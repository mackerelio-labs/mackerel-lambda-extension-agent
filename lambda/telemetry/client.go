...
func NewClient(runtimeApi string) *Client {
	baseUrl := fmt.Sprintf("http://%s/2022-07-01/telemetry", runtimeApi)
	return &Client{
		httpClient: &http.Client{},
		baseUrl:    baseUrl,
	}
...
func (c *Client) Subscribe(ctx context.Context, extensionId string, listenerUri string) (*SubscribeResponse, error) {
	...
	Logger.Info("Subscribing using baseUrl:", c.baseUrl)
	...
	if resp.StatusCode == http.StatusAccepted {
		Logger.Warning("Subscription failed. Logs API is not supported! Is this extension running in a local sandbox?")
	...
	Logger.Info("Subscription success:", string(body))
